/// <reference path="../types/podlet.d.ts" />
/* eslint-disable max-classes-per-file */
/* eslint-disable no-param-reassign */
import { destinationObjectStream } from '@podium/test-utils';
// @ts-ignore template is exported, but not in the type definitions at time of writing
...
class FakeExpressServer {
     ...
     *
     * @param {Podlet} podlet
     * @param {import('express').RequestHandler | null} [onRequest]
     * @param {{ path: string; handler: import('express').RequestHandler} | import('express').RequestHandler | null} [onContentRoute]
     * @param {import('express').RequestHandler} [onFallbackRoute]
     */
...
tap.test('Podlet() - no value given to "name" argument - should throw', (t) => {
    t.throws(() => {
        // @ts-ignore - we are testing invalid values here
...
tap.test(
    ...
    async (t) => {
        const podlet = new Podlet(DEFAULT_OPTIONS);
        // @ts-ignore - good enough for the unit test
