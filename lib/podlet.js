...
const PodiumPodlet = class PodiumPodlet {
    ...
    async process(incoming) {
        ...
        if (
            s.request.headers['user-agent'] &&
            s.request.headers['user-agent'].startsWith('@podium/client')
        ) {
            s.development = false;
        } else {
            s.development = this.development;
        }
        ...
        if (s.development) {
            ...
            s.context = utils.deserializeContext(incoming.request.headers);
            this.log.debug(
            ...
            this.process(incoming)
                ...
                .then(result => {
                    if (result) {
                        ...
                        res.podiumSend = data =>
                            res.send(this.render(incoming, data));
