...
const PodiumPodlet = class PodiumPodlet {
    ...
    [_default]() {
        return (req, res, next) => {
            if (this.useDefaults) {
                ...
                const pathname = this.manifestRoute.split('/').map(item => item.endsWith('.json') ? '' : item).join('/');
                ...
                const context = Object.assign({
                    ...
                    mountPathname: pathname,
                    publicPathname: pathname,
                }, this.defaultContext);

                utils.setAtLocalsPodium(res, 'context', context);

                this.log.debug(`Appending a default context to inbound request "${JSON.stringify(context)}"`);
            }
    ...
    [_context]() {
        return (req, res, next) => {
            ...
            this.log.debug(
                `Inbound request contains a context "${JSON.stringify(context)}"`
            );
            ...
            if (this.useDefaults) {
                ...
                this.log.debug(
                    `Merged default context with context on inbound request "${JSON.stringify(context)}"`
                );
            }
    ...
    [_template]() {
        return (req, res, next) => {
            ...
            if (req.get('user-agent') && req.get('user-agent').startsWith('@podium/client')) {
                this.log.debug(
