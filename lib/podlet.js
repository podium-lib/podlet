...
export default class PodiumPodlet {
    ...
    /**
     * I'm imaginging this as a helper method to be used in conjunction with render/podiumSend.
     *
     * Eg.
     * app.get("/", (req, res) => {
     ...
     * 1. This gives the podlet developer full control of when to isolate the content without automagic.
     * 2. This enables podiumSend/render to continue working exactly as they are making this a fully backwards compatible feature.
     *
     ...
     */
    isolate(incoming, data, { mode = 'open' }) {
        ...
        // We can leverage the new asset scope feature and add a new scope type of 'shadow-dom' to make it possible to filter out shadow dom assets for inclusion.
				// The podlet developer can then do something like podlet.css(new AssetCSS({ scope: 'shadow-dom', value: 'https://foo.com/bar.css' }));
    ...
    middleware() {
        return async (req, res, next) => {
            ...
            try {
                ...
                // naming??
                res.podiumIsolate = (data, ...args) =>
                    this.isolate(incoming, data, ...args);
