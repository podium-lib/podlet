...
const PodiumPodlet = class PodiumPodlet {
    ...
    /*
    render(fragment, res) {
        if (utils.getFromLocalsPodium(res, 'development')) {
...
*/
    ...
    process(req, res) {
        ...
        return new Promise((resolve, reject) => {
            state.view = this._view;
            state.name = this.name;
            state.css = this.cssRoute;
            state.js = this.jsRoute;

            // Determin if request comes from layout server or not
            if (
                state.request.headers['user-agent'] &&
                state.request.headers['user-agent'].startsWith('@podium/client')
            ) {
                state.development = false;
            } else {
                state.development = this.development;
            }
            ...
            if (this.development) {
                ...
                state.context = utils.deserializeContext(state.request.headers);;
                this.log.debug(
                    `Inbound request contains a context "${JSON.stringify(state.context)}"`
                );
            }
    ...
    middleware() {
        return (req, res, next) => {
            this.process(req, res)
                .then(state => this._proxy.process(state))
                .then(state => {
                    if (!state) {
                        return;
                    }
                    ...
                    objobj.set('locals.podium', state, res);
