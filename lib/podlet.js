...
const PodiumPodlet = class PodiumPodlet {
    ...
    async process(incoming, { proxy = true } = {}) {
        ...
        // if we are outputting a custom element, we need the scripts/styles inside shadow dom
        // so we can't send via manifest file.
        if (!this.element) {
        ...
        return async (req, res, next) => {
            ...
            try {
                ...
                res.podiumSend = (data, ...args) => {
                    ...
                    if (this.element) {
                        // wrap the content in shadow dom
                        // include the podlet's css and js
                        content = el(`
                            ${this.cssRoute
                                .map((css) => css.toHTML())
                                .join('\n')}
                            ${data}
                            ${this.jsRoute.map((js) => js.toHTML()).join('\n')}
                        `);
                    }

                    res.send(this.render(incoming, content, ...args));
                };
